angular.module('leviticus').service('WikiImage',['config',function(config) {


	this.getImageTag = function(args) {
		if (!args) args = {};
		if (!args.name) { error('WikiImage.getImage > cannot get image with no name.'); return ''; }
		if (!args.article) { error('WikiImage.getImage > article not supplied.'); return ''; }
		...
		if (!image) {
			...
			var imgUrl = config.getImageRoot() + image.path;		
			// style
